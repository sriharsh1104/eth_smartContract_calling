...
const SelectTokenModal = (props) => {
  ...
  console.log("ggggggggggggggg", tokenCollection.filter((item) => item.tokenName !== selectedtoken1?.tokenName))
  let tokenlisttwo = tokenCollection.filter((item) => item.tokenName !== selectedtoken1?.tokenName)
  let tokenlistone = tokenCollection.filter((item) => item.tokenName !== selectedtoken2?.tokenName)
...
const TokentList = ({ hname, children }) => {
  return (
    <div className="tokenList">
      <div className="tokenList_head">
        {/* <span>{hname} Name</span>
        <span>Balance</span> */}
      </div>
    </div>
  );
...
const TokenListItem = ({
  ...
  const getBAlance = async (address) => {
    const tokenA = TokenCollection.filter(
      (a) => a.symbol === "TKNA" || "TKNB" 
    );
    if(tokenA.isNative){
      ...
      const tokenAbalance = await contractA.methods.balanceOf(waddress[0]).call();
