...
import { RemoveLiquidityETHWithPermitService, RemoveLiquidityWithPermitService } from "../../../AddLiquidity/addLiquidity";
...
const RemoveLiqInModal = (props) => {
  ...
  const pairAddressinput = useSelector((state) => state.tokenAction.pairAddress);
  const tokenASelected = useSelector((state) => state.tokenSelectedSlice.tokenA);
  const tokenBSelected = useSelector((state) => state.tokenSelectedSlice.tokenB);


  // const calculateValues = async () => {
  //   const totalLiquidity = await pairBalanceHelper(waddress[0], pairAddressinput);
  //   const result = await getReservesHelper(pairAddressinput);
  //   console.log("resullllllllllllt", result);
  ...
  //   console.log("tokenAreserve", reserve1, reserve2);
  ...
  //   const balance = await pairBalanceHelper(waddress[0], pairAddressinput);
  //   if (balance) {
  //     try{      
  //     const result = await RemoveLiquidity(tokenASelected.address, tokenBSelected.address, balance, 1, 1, waddress[0], deadline)
  //     if (result?.status) {
  ...
  const signRemoveLiquidityTxn = async () => {
    ...
    deadline = deadline + 10 * 60;
    const totalLiquidity = await pairBalanceHelper(waddress[0], pairAddressinput);
    const result = await getRemoveLiquiditySignature(waddress[0], pairAddressinput, BigNumber(totalLiquidity), 1685180759);
  ...
  const removeLiquidityWithPermit = async () => {
    ...
    const customToken = tokenASelected.isNative ? tokenBSelected : tokenASelected;
    const totalLiquidity = await pairBalanceHelper(waddress[0], pairAddressinput);
    if (tokenASelected.isNative || tokenBSelected.isNative) {
      try{
        console.log("fasdfjnldasfj")
        const result = await RemoveLiquidityETHWithPermitService(waddress[0], signature, BigNumber(totalLiquidity), "0", "0", customToken.address, 1685180759);
        if (result?.status) {
      ...
      const totalLiquidity = await pairBalanceHelper(waddress[0], pairAddressinput);
      const result = await RemoveLiquidityWithPermitService(waddress[0], signature, BigNumber(totalLiquidity), "0", "0", tokenASelected.address, tokenBSelected.address, 1685180759);
      if (result?.status) {
        toast.success("Liquidity Removed")
      } 
    }catch(err){
  ...
  return (
    ...
    >
      ...
      <ButtonCommon title="Approve" onClick={signRemoveLiquidityTxn}></ButtonCommon>
      <ButtonCommon title="Remove Liquidity" onClick={removeLiquidityWithPermit}></ButtonCommon>
