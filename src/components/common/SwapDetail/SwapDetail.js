...
const SwapDetail = ({ priceImpact }) => {
  ...
  const SwappableFun = async () => {
    // console.log("result123", selectedtoken1.address);
    // console.log("result123", selectedtoken2.address);
    const result2 = await GetAmountsOut(inputSwap, [path[0], path[1]]);
    setSwappable(result2[1]);
  };
  // const MiniReceivedDisplay = async() => {
  ...
  return (
    <ul className="swapDetail">
      <li>
        ...
        {
          priceImpact > 0 ? <span className="positive">{priceImpact}%</span> : <span className="negative">{priceImpact}%</span>
      ...
      <li>
        <span className="swapTitle">
          ...
          <Tooltip
            placement="bottom"
            title="This fee is only taken when bYSL is either purchased (10% fee) or sold (10% fee)."
          >
            <span className="swapInfo_Icon">
              <InfoIcon />
            </span>
