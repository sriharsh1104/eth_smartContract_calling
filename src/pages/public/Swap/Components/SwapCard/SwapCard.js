...
const SwapCard = ({ mode, handleToggle, setPriceImpact }) => {
    ...
    if ((selectedtoken1.address && selectedtoken2.address)) {
      ...
      if (input1 == "") {
        ...
        if (selectedtoken1.isNative) {
          ...
          const result = await GetAmountsOut(input1, array);
          console.log("resuleeeeeeeee", result);
          dispatch(setInputValue2(result[1]));
        }
      }
    }
  ...
  const getPairAddress = async () => {
    ...
    // console.log({reserveOne,reserveTwo});
    ...
    setBeforeSwapReserves({
      ...
      reserveOne: await token1.methods.balanceOf(finalResult).call() / 10 ** 18,
      reserveTwo: await token2.methods.balanceOf(finalResult).call() / 10 ** 18,
    });
  };
  ...
  const AmountsOut = async (amountIn, reservesOut, reserveIn) => {
    // console.log({ amountIn, reserveIn, reservesOut });
    let amountToken2 =
  ...
  const getAmountsOnOne = async () => {
    console.log("beforeSwapReserves", beforeSwapReserves.reserveTwo,
      beforeSwapReserves.reserveOne)
    ...
    console.log({ beforeAmountOne, AfterAmountOne });
    getPriceImpact(beforeAmountOne, AfterAmountOne);
    console.log({ beforeSwapReserves });
    console.log({ afterSwapReserves });
  };

  const getPriceImpact = (beforeAmountOne, AfterAmountOne) => {
    let finalAmount = (beforeAmountOne - AfterAmountOne) != 0 ? ((beforeAmountOne - AfterAmountOne) * 100) / beforeAmountOne : 0;
    // let finalAmount =
  ...
  console.log("tokenBalanceA", Number(tokenBalanceA / 10 ** 18).toFixed(3))
