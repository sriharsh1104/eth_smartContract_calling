...
import { PlusIcon, RefreshIcon, SettingIcon, } from "../../../../../assets/Svg/SvgImages";
import CoinInputLiquidity from "../../../../../components/common/Inputs/coinInputLiquidity";
import { allowanceHelper, approvalHelper, getTokenAddress, pairBalanceHelper } from "../../../../../Swapping/Swap";
...
import { AddLiquidity, AddLiquidityEth, RemoveLiquidity } from "../../../../../components/AddLiquidity/addLiquidity";
import { getPairHelper, pairApprovalHelper, pairAllowanceHelper } from "../../../../../Swapping/Swap";
...
const SwapCard = ({ mode }) => {
  ...
  const selectedtoken1 = useSelector((state) => state?.tokenSelectedSlice?.tokenA)
  const selectedtoken2 = useSelector((state) => state?.tokenSelectedSlice?.tokenB)
  ...
  const balanceOfA = async () => {
    if (selectedtoken1.isNative) {
      ...
      let ethBalance = await web3Object.eth.getBalance(waddress[0]);
      ...
      let result = await contract.methods.balanceOf(waddress[0]).call();
  ...
  const balanceOfB = async () => {
    ...
    if (selectedtoken2.isNative) {
      ...
      console.log("ethBalance1", convertedAmount)
      ...
      console.log("ethBalance12", result)
      return result;
    ...
    if (selectedtoken1.address && selectedtoken2.address) {
      ...
      if (selectedField == "Field1") {
        if (input1 == '') {
          ...
          console.log("contract1",contract1,contract2);
          ...
          if (selectedtoken1.address == token0) {
            ...
            console.log("rssssssss",result);
            dispatch(setInputValue2Liquidity(result))
          }
          else {
            console.log("elseleele");
  ...
  const removeLiquidity = async () => {
    ...
    if (balance) {
      const result = await RemoveLiquidity(
        selectedtoken1.symbol, selectedtoken2.symbol, balance, 1, 1, to, deadLine
      );
      if (result?.status) {
  ...
  const handleRemoveLiquidity = async () => {
    const pairAddress = await getPairHelper(selectedtoken1.symbol, selectedtoken2.symbol)
    const balanceOfpair = await pairBalanceHelper(waddress[0], pairAddress);
    const allowanceResult = await pairAllowanceHelper(waddress, "PAIR", pairAddress);
