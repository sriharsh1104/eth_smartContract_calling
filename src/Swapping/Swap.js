...
export const SwapExactTokenForTokenHelper = async (
    ...
    path,
    to,
    deadLine, selectedGasPrice, slippageTolerance
) => {
    ...
    const input2WithSlippage = amountOutMin - (amountOutMin / 100) * slippageTolerance;
    const newvalue = BigNumber(input2WithSlippage);
    if (contract) {
        try {
            let gasLimit;
            console.log("assdd")
            gasLimit = await contract.methods.swapExactTokensForTokens(
                amountIn,
                newvalue,
                path,
                to,
            ...
            console.log("assddestimation",gasLimit)
            ...
            const result = await contract.methods.swapExactTokensForTokens(
                ...
                .send({ from: waddress, gasPrice: selectedGasPrice * 10 ** 9, gas: gasLimit });
            return result;
        }
        catch (err) {
            if(err.code == 4001){
                ...
                let newSlippage = 50;
                console.log("heqwerty", newSlippage, waddress, slippageTolerance);
                try {
                    let gasLimit;
                    const taxedValue = BigNumber(amountOutMin - (amountOutMin / 100) * newSlippage);
                    gasLimit = await contract.methods.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                        amountIn,
                        taxedValue,
                        path,
                        to,
                    ...
                    console.log("gassslimit", gasLimit);
                    const result = await contract.methods.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                        amountIn,
                        taxedValue,
                        path,
                        to,
...
export const SwapTokenForExactTokenHelper = async (
    ...
    const router = uniswapContractsInfo.filter((a) => a.symbol === "ROUTER")
    const input1WithSlippage = (amountInMax / 100) * slippageTolerance + amountInMax;
    const newvalue = localeStringFunction(input1WithSlippage);
    const contract = await Web3Instance(router[0].abi, router[0].address);
    const result = await contract.methods.swapTokensForExactTokens(
        amountOut,
        newvalue,
        path,
        to,
...
export const SwapExactTokensForEthHelper = async (
    ...
    if (contract) {
        try {
            let gasLimit;
            gasLimit = await contract.methods.swapExactTokensForETH(
                amountIn,
                newvalue,
                path,
                to,
            ...
            const result = await contract.methods.swapExactTokensForETH(
                amountIn,
                amountOut.toString(),
                path,
                to,
            ...
            if(err.code == 4001){
                ...
                try {
                    let gasLimit;
                    const taxedValue = BigNumber(amountOut - (amountOut / 100) * newSlippage);
                    gasLimit = await contract.methods.swapExactTokensForETHSupportingFeeOnTransferTokens(
                        amountIn,
                        taxedValue,
                        path,
                        to,
...
export const SwapEthForExactTokenHelper = async (
    ...
    const result = await contract.methods.swapETHForExactTokens(
        newvalue,
        path,
        to,
        deadLine
...
export const SwapExactEthForTokenHelper = async (
    ...
    if (contract) {
        try {
            let gasLimit;
            gasLimit = await contract.methods.swapExactETHForTokens(
                newvalue,
                path,
                to,
                deadLine
            ...
            const result = await contract.methods.swapExactETHForTokens(
                ...
                .send({ from: waddress, value: amountIn, gasPrice: selectedGasPrice * 10 ** 9 });
            return result;
        }
        catch (err) {
            if(err.code == 4001){
                ...
                try {
                    let gasLimit;
                    const taxedValue = BigNumber(amountOutMin - (amountOutMin / 100) * newSlippage);
                    gasLimit = await contract.methods.swapExactETHForTokensSupportingFeeOnTransferTokens(
                        taxedValue,
                        path,
                        to,
                        deadLine
...
export const pairAllowanceHelper = async (waddress, tokenName, pairAddress) => {
    ...
    const result = await contract.methods.allowance(waddress.toString(), router[0]?.address).call();
    return result;
}
...
export const pairApprovalHelper = async (waddress, tokenName, pairAddress) => {
    ...
    if (contract) {
        const result = await contract.methods.approve(router[0].address, maxApproval).send({ from: waddress[0] })
        return result;
    }
}
...
export const getReservesHelper = async (pairAddress) => {
    ...
    if (contract) {
        ...
        if (address == tokenInfo[0].address) {
            ...
            const { _reserve1, _reserve0 } = await contract.methods.getReserves().call();
